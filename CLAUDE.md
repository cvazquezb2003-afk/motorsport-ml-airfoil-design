...
> Lo que queda es **infraestructura** (repo, despliegue), no núcleo.
...
| `circuitos.py` + `circuitos.csv` | **42 circuitos** → nivel de downforce → rango de |α|. Atajo, con nota de equivalencia |
| `curvas_optimo.py` | Curvas predichas del óptimo (3 velocidades) y **L/D vs velocidad** |
| `optimo_geom.py` | `.dat` del óptimo + **redondeo del TE a 0.05 mm** + Cp (fallback al vecino) + cachés |
...
> Recorrido de la carpeta. Conteo **verificado el 2026-08-06**: **96 `.py`, 19 `.csv`,
> 238 `.json`, 4 `.joblib`, 9 `.png`, 4 `.md`** + `dataset_runs/` (1.293 carpetas),
...
> Lo visual y lo de producto **ya está**. Lo que queda es **infraestructura**, no núcleo.
...
| **GitHub** | ❌ **sin `.git`, `.gitignore`, `README`, `requirements.txt`** — y ⛔ **BLOQUEADO** por el ensemble de 111 MB (ver abajo) | 🟢 bajo |
| **Persistencia de diseños en servidor** | 🟡 hoy es `localStorage` del navegador (decisión consciente: cero infra) | 🟡 medio |
| **Despliegue** (fuera de `localhost`) | ❌ no hecho | 🟡 medio |

### ⛔ CABO SUELTO — el ensemble YA NO CABE en GitHub (bloquea el primer push)

> **Esto pasó de "aviso" a BLOQUEO con la promoción densif.** Antes eran 64 MB: molesto
> (GitHub avisa a partir de 50) pero subible. Ahora son **111 MB y el límite duro de
> GitHub es 100 MB**: el push se RECHAZA, no es un warning. Hay que resolverlo antes del
> primer commit, no después.

- **`ensemble_ld_sigma.joblib` pesa 111 MB** (era 66,4 MB antes de la densificación).
  Creció porque los árboles tienen más nodos al entrenar con ~4× filas; la profundidad
  sigue capada en `max_depth=10`, por eso **no** es más lento (medido: en lote de 32.768
  candidatos es incluso un 10% más rápido que el viejo).
- Tres salidas posibles, en orden de preferencia:
  1. **`.gitignore` + regenerar en destino** con `winner_curse.py` (~1 min de cómputo).
     Es el artefacto más reproducible del proyecto y no aporta nada versionarlo.
  2. **Git LFS** si se quiere binario exacto reproducible sin reentrenar.
  3. Comprimir / reducir `M` (miembros del ensemble), pero eso **cambia σ** y obligaría a
     revalidar la batería. Descartable salvo necesidad.
- **`dataset_runs/` tiene 1.293 carpetas** y `airfoil_dataset_densif_merged.csv` pesa
  14,1 MB (subible, pero conviene decidir si va al repo o se regenera).
- Hace falta un **`.gitignore` bien pensado** antes del primer commit. Si no, el repo
  nace envenenado.
...
**Dashboard / layout**
- [ ] **CSV descargable** del resultado (hoy solo hay `.dat` y la ficha JSON/TXT).
- [ ] **Reducir el texto de Results**: se ha ido acumulando explicación (franja, catálogo,
      avisos, cargas) y el panel pesa demasiado para un primer vistazo. Mover parte al
      patrón de tooltip `?` que ya se usa en Compare.
- [ ] **Aclarar dos mensajes que se malinterpretan:**
      - *"better than 90% of existing profiles"* — no queda claro que el 90% es un
        **percentil frente al catálogo medido**, no una mejora del 90%.
      - *"closest to database"* (el vecino fabricable) — no queda claro que es
        **referencia de contexto**, no la propuesta que se entrega.
- [ ] **Revisar la clasificación de circuitos**: los 42 de `circuitos.csv` se agrupan en
      low/medium/high a ojo. Conviene contrastarla con datos reales de reglaje o, al
      menos, documentar el criterio y suavizar el lenguaje.
- [ ] **Alinear la vista Compare** con Results: la tabla y las notas han crecido por
      separado y ya no comparten estructura ni jerarquía visual.
...
**Geometría / CATIA**
- [ ] **Checks del `.dat` generado en CATIA**: verificaciones automáticas de la geometría
      exportada (auto-intersección, cierre del TE, monotonía en x, nº de puntos) antes de
      mandarla a XFOIL. Hoy se detectan a posteriori o a ojo — y ya han costado dos
      episodios: el kink del TE y el `.dat` amputado del archivo.
