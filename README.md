...
CATIA ──► point cloud ──► .dat ──► XFOIL ──► dataset ──► surrogates ──► inverse design
 (geometry)                        (truth)              (XGBoost)     (differential evolution)
```
...
**3. Inverse design.** Given a target, `differential_evolution` (SciPy) searches the 7-parameter space, bounded to the p5–p95 region actually covered by the data. The objective is not the raw prediction:
...
- **Inverse design**: Sobol sampling with a fixed seed (32,768 points) feeding
  `differential_evolution`. The same target returns the same geometry.
...
**Python** · **Flask** (web app, no framework beyond it) · **XGBoost** (surrogates and uncertainty ensemble) · **SciPy** (`differential_evolution`, Sobol sampling, spline fitting) · **XFOIL** (viscous panel solver, ground truth) · **Plotly** (interactive charts) · **CATIA V5** (parametric geometry, generation only — not needed to run the app) · **pandas / NumPy** · **Docker**
